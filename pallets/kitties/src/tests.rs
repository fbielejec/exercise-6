...
#[test]
fn can_transfer() {
    // update this test to check the updated behaviour regards to KittyPrices
    new_test_ext().execute_with(|| {
        assert_ok!(KittiesModule::create(Origin::signed(100)));
        ...
        // kity not for sale after transferring
...
fn can_set_price() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_last_event(Event::KittiesModule(
            crate::Event::<Test>::KittyPriceUpdated(100u64, 0u32, Some(10)),
        ));
    });
}
...
fn can_buy() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            KittiesModule::buy(Origin::signed(100), 100, 0, 11),
            Error::<Test>::BuyFromSelf
        );

        assert_noop!(
            KittiesModule::buy(Origin::signed(101), 100, 0, 9),
            Error::<Test>::PriceTooLow
        );

        assert_ok!(KittiesModule::buy(Origin::signed(101), 100, 0, 11));

        System::assert_last_event(Event::KittiesModule(crate::Event::<Test>::KittySold(
            100u64, 101, 0u32, 10,
        )));
    });
}
